---
name: execute-plan
description: Execute an approved plan through isolated implementation, verification, commits, and PR delivery. Use --full-cycle only for requested pre/post review; small single edits do not need this workflow.
---
...
- **Milestone commits.** Commit each meaningful chunk as it lands. One commit is sufficient for one cohesive milestone. Use the natural seams in the plan.
...
7. **Full-cycle post-review + e2e** (with `--full-cycle` or an explicit post-review request). Review exactly the committed cumulative delta since `EXEC_BASE`. Before each post-review, verify every plan-created file and pending fix, make the corresponding focused milestone or review-fix commit, and confirm a clean worktree; never rely on a reviewer to discover staged or untracked files implicitly. Use `review-cycle` when the branch diff equals that execution delta — the normal case for the fresh worktree created in Step 1. Otherwise the range is wrong: fall back to `tribunal-review --base <EXEC_BASE>` and disclose the downgrade. If a review applies further fixes, verify them, create a focused review-fix commit, and re-run the same reviewer before reporting done. If the plan defines an end-to-end validation (a smoke test, a real API call, a real query), run it now against real data — distinguish observed data from labeled fixtures or simulation; a fixture run cannot prove live integration. Add the reviewer, verdict, `EXEC_BASE`, and e2e result as rows in the summary table from Step 6. The verdict is a gate, not a note: `APPROVE`/`SHIP` completes the run; `CHANGES NEEDED`/`FIX-FIRST` means apply the findings, re-verify, commit, and re-review; a finding that invalidates the approach stops the run and goes to the user.

8. **Remote delivery.** Delivery is part of the run, not a follow-up question. Unless the invocation said otherwise, the run ends with the branch pushed and a PR opened — or the existing PR updated — even when the plan omits that mechanical step, and the PR URL goes into the summary table as a row. Prepare the final table after delivery. Before any push/PR, verify the current repo is the plan's target repo (`git remote -v` / cwd) and the branch diff contains only the intended delivery — a mismatch is a hard stop. Never push directly to `master`/`main`.
...
- **If a milestone blows up the scope.** Stop, surface what you found, ask. Don't silently expand the plan. Treat corrections, clarifications, and status questions as steering of the same task. Update the remaining plan when needed; ask only about genuinely new scope or changed authorization, not for a new invocation.
